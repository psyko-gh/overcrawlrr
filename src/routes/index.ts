...
apiRouter.use('/healthcheck', async (req, res, next) => {
    res.status(200).json({
        status: 'ok'
    })
});

apiRouter.use('/movies/upcoming', async (req, res, next) => {
...
apiRouter.use('/movies/popular', async (req, res, next) => {
    const overseerr = new OverseerrApi();
    await overseerr.auth();
...
apiRouter.use('/search', async (req, res, next) => {
    const overseerr = new OverseerrApi();
    await overseerr.auth();
    const data = await overseerr.getUpcoming();
    res.status(200).json(data)
...
apiRouter.use('/movies/:id(\\d+)/evaluate/:ruleset', async (req, res, next) => {
    try {
        const overseerr = new OverseerrApi();
        await overseerr.auth();
...
apiRouter.use('/movies/:id(\\d+)/request', async (req, res, next) => {
    const overseerr = new OverseerrApi();
    await overseerr.auth();
...
apiRouter.use('/movies/:id(\\d+)', async (req, res, next) => {
    const overseerr = new OverseerrApi();
    await overseerr.auth();
...
apiRouter.use('/discover', async (req, res, next) => {
    try {
        await discover()
        res.status(200).json({
            success: true,
...
apiRouter.use('/smartRecommendations', async (req, res, next) => {
    try {
